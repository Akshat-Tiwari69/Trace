...
| **A10** | ✅ | **Mask post-processing module** | — | S3, E2 | New `p1_segment/postprocess.py`: morphological open/close, connected-component area filter, Otsu option, skeleton-spur pruning; called by `predict.py`. Done when tiny false components drop on a sample tile without IoU loss > 0.005. — **DONE:** `postprocess.py` with `remove_small_components` (lossless on the real network), `morphological_cleanup` (open/close), `otsu_threshold`, `prune_skeleton_spurs`, and a conservative `postprocess_mask` chain (default = drop small components only, IoU-safe on thin roads). Wired into `predict.py` via `--postprocess` / `--min-component-size` / `--pp-close-radius`. **Criterion verified on a real Panaji tile (512², 8.4% road): components 136→2, IoU vs GT 0.9938→0.9999 (loss +0.0001 ≤ 0.005).** 9 unit tests; full suite 151 green. |
...
| **A12** | ✅ | **Topology-aware self-training on Indian tiles** *(tested → **REJECTED** 2026-06-28; pivot to A17/A18)* | A6, A11, **A16** | A13 | **UniMatch/UniMatch-V2 weak-to-strong consistency + connectivity-refined pseudo-labels** (skeleton-prediction aux task; drop discontinuous pseudo-labels) over the unlabeled Indian OSM corpus (A16), then anti-forgetting fine-tune on clean Indian GT (A17); optional RoadDA adversarial booster. Done when **APLS** (+ Indian-GT IoU) beats v2 on the **hand-labeled** Indian set (A17), validated each self-train round to avoid noise amplification. Refs: `LiheYoung/UniMatch`, `LANMNG/RoadDA`. — **HARNESS DONE:** `train_selftrain.py` — EMA mean-teacher; labeled stream (DeepGlobe + SpaceNet-5 Mumbai) supervised, unlabeled Indian corpus via teacher weak→student strong **consistency** (same-geometry views), **confidence-gated** + **connectivity-refined pseudo-labels** (`refine_pseudo` drops small disconnected blobs via `scipy.ndimage` so noise isn't amplified), λ-rampup. 4 CPU smoke tests (two-view dataset, refine, masked-consistency, 1-epoch train+save). **RESULT — REJECTED (2026-06-28):** ran on the 3070 Ti (warm-start v1; labeled DeepGlobe+SpaceNet, unlabeled Indian OSM corpus; val = margao+delhi, TEST = panaji/mumbai_bandra/bengaluru kept untouched). Both configs — original (consistency λ→1.0) and a gentler re-tune (λ→0.3, conf 0.85) — **peaked at epoch 1 then declined every epoch** on val. **Honest held-out TEST eval (IoU @0.44, 384px, single-view): v1 0.281 vs A12-best 0.258 (re-tune) / 0.243 (original) → A12 LOSES.** Mean-teacher self-training on OSM-weak labels adds no real signal on the deployment target (joins A8/A9/A11). The OSM-agreement *val* metric proved **misleading** (a higher val IoU still lost on TEST). **Not released — v1 stays the clean baseline, v2 stays deployed.** Pivot → **A17** (hand-labeled Indian GT for a truthful APLS/IoU metric) **/ A18** (graph-first SAM-Road++). Detail in §10 (2026-06-28). |
...
| **A17** | ✅ | **Truthful Indian GT benchmark via held-out SpaceNet-5 Mumbai (no hand-labeling)** | A16 | E2, A23 | **REFRAMED 2026-06-28 — hand-labeling is off the table** (team are students; drawing vector centerlines for 50–150 tiles isn't realistic), **and we don't need to:** **SpaceNet-5 Mumbai (built in A16) already IS professional, human-drawn Indian GT** — 2,207 tiles + the **raw vector centerline GeoJSONs** (`data/raw/spacenet/SN5_roads_train_AOI_8_Mumbai/geojson_roads_speed/`, 0.3 m). Plan: fix a **held-out Mumbai TEST split** (never trained on) → **IoU on masks + APLS on the vector GT** (reuse S7 `p3_analysis/apls.py`). Becomes the **headline** Indian metric — OSM-agreement was proven *misleading* in A12 (it rewarded models that lose on held-out). **Done when:** a fixed held-out SpaceNet-Mumbai split + an IoU/APLS eval script exist and **v1/v2 are re-baselined on real GT** (we have never measured on real Indian GT — even A12's "loses to v1" was scored on OSM-labeled `data/finetune` tiles). Caveat: single city (Mumbai) but real GT + standard benchmark. *(Original hand-label/active-learning idea + a working tile-selector are parked on branch `akshat/A17-tile-selector` if labeling capacity ever appears.)* Lane: Akshat (+ S7 APLS). — **IoU HALF DONE (2026-06-28):** `eval_spacenet.py` + frozen **chip-level** held-out split (`data/sample/spacenet_mumbai_heldout_chips.json`, seed 17, 127/637 chips = 449 tiles, never trained on) + IoU/Dice eval (reuses `train.evaluate`). 4 unit tests. **First real-GT Indian re-baseline (held-out, @0.44, 512px): v1 IoU 0.3768 ≈ v2 0.3727 — v2 does NOT beat v1 on real GT** (v2's apparent edge was an OSM-agreement artifact on its own training tiles → A6 fine-tune bought ~nothing on truth). **v1 0.377 is now the honest bar for A23.** Report: `data/sample/spacenet_mumbai_eval.json`. — **APLS HALF DONE → A17 ✅ (2026-06-28):** `apls_eval.py` — per-tile mask→skeleton→graph (P2 `skeleton_to_graph`) → **S7 `apls.py`** (node coords rescaled to the lon/lat form apls projects; empty-pred graph guarded). **Held-out APLS (topology/routing, n=50 tiles): v3 (road_spacenet) 0.4147 vs v1 0.3844 → +0.030 (+7.9%)** — v3 wins on routing, not just pixels. 3 unit tests (identical-mask≈1.0, empty-GT skip). Report `data/sample/spacenet_mumbai_apls.json`. **Both halves done: truthful Indian benchmark = held-out SpaceNet-Mumbai IoU + APLS (RGB + Cartosat grayscale proxy), v1/v2 re-baselined, v3 validated.** |
...
| **A21** | ✅ | **Prediction-threshold sweep in the honest eval** *(review #9)* | — | — | The v4-vs-v1 eval fixes thr 0.44; sweep per-model best threshold (fairly, same protocol/resolution for both) and report at each model's own optimum **and** at a shared threshold. Done when the A12 eval reports both, so the verdict isn't an artifact of one fixed threshold. Cheap; fold into `scratchpad/eval_*`/`evaluate.py`. Lane: Akshat. — **DONE (2026-07-02):** `eval_spacenet --sweep` (0.20–0.70, one inference/tile via `predict_prob`, cheap threshold loop; reports each model's best + @0.44). **Held-out sweep: best thr = 0.50 for BOTH v3 and v1** (0.44 was slightly low). @0.50 RGB: v3 **0.4493** vs v1 **0.3993**; grayscale: v3 0.4046 vs v1 0.3447 — v3 wins at every threshold. Also refreshed all held-out JSONs to the **global** metric (A25): RGB v3 0.4311 / v1 0.3752, grayscale v3 0.3752 / v1 0.3183 (win robust, +15%). **Recommend bumping the deploy threshold 0.44→0.50** (~+0.02 IoU free). Reports: `spacenet_mumbai_threshold_sweep{,_gray}.json`. **DONE — acted on it (2026-07-02): released `a4-roadseg-v3.1` (v3 weights, meta threshold 0.50; +0.018 IoU); now the deployed default.** |
...
| **A23** | ✅ | **Supervised fine-tune on SpaceNet-5 Mumbai (real Indian GT) → eval on held-out** — **WIN: first model to beat v1 on real GT** *(the post-A12 lever)* | A16, A17 | — | Replaces the rejected OSM self-training (A12) with a **clean supervised path**: fine-tune **v1** on the SpaceNet-Mumbai TRAIN split (strong human labels, **not** noisy OSM), anti-forgetting via a DeepGlobe anchor (as A6 did), and evaluate on the **A17 held-out Mumbai TEST split** (IoU + APLS) — a metric we can actually trust. **Release only if it beats v1 on the truthful metric.** Fully automated, no hand-labeling; GPU run (3070 Ti / Kaggle). **MUST include the A24 sensor-robustness aug** (grayscale/radiometric) so it generalises to Cartosat-3 PAN, not just WorldView-3 RGB — track both RGB *and* grayscale-proxy IoU each run. Lane: Akshat. — **HARNESS DONE + RUNNING (2026-06-28):** reused A6 `finetune.py` (frozen encoder + DeepGlobe-2000 anchor) with 3 surgical adds: `finetune_pairs` override (feeds SpaceNet **train** split = non-held-out chips via `eval_spacenet.train_pairs`), `grayscale_p=0.5` aug (A24), `cldice_weight` configurable (off → avoids the 8 GB clDice OOM). Live run `road_spacenet.pt`: v1 init, 1758 SpaceNet-train + 2000 DG anchor, 512px, batch 2, frozen encoder, 10 ep, grayscale 0.5. **RESULT — ✅ WIN (2026-06-28):** best epoch 6. **Honest held-out eval (127 chips/449 tiles, real GT, @0.44, 512px):** RGB **A23 0.4298 vs v1 0.3768 (+0.053, +14%)**; grayscale Cartosat-PAN proxy **A23 0.3811 vs v1 0.3235 (+0.058)**. Also beats v2 (0.3727). DeepGlobe preserved (0.6802 ≥ v1). **Sensor gap shrank** (A23 grayscale drop −11.3% vs v1's −14.1%; A23-gray 0.3811 ≈ v1-RGB 0.3768). **First genuine improvement over v1 on real Indian GT** after A8/A9/A11/A12 all failed — the lever was real in-domain labels + a truthful metric, exactly the thesis. `models/road_spacenet.pt` (gitignored); reports `data/sample/spacenet_mumbai_eval{,_gray}.json`. ****Released as `a4-roadseg-v3`** (https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3)** (tag/name TBD). Remaining: APLS (A17), optional encoder-unfreeze to push further. |
| **A24** | ✅ | **Sensor-robustness for Cartosat-3 (grayscale/radiometric aug) + PAN-proxy eval** *(deployment-critical)* | A17 | A23 | **The real deployment input is Cartosat-3** (PRD G1; provided at hackathon end, restricted) — and per Bhoonidhi it ships as **PAN (panchromatic ~0.25 m grayscale)** / MX (1.13 m, 4-band) / MX-Merge (pan-sharpened). Our whole pipeline is **RGB** → a sensor-modality gap we don't yet train for. **Measured (2026-06-28):** v1 on held-out SpaceNet Mumbai drops **RGB 0.375 → grayscale 0.318 (−15 % rel)** — the model leans on colour it won't have on PAN (real gap likely larger). **Fix:** add **`A.ToGray` + radiometric/contrast aug** to the train transform (`build_train_transform`) so the model can't depend on colour; verify the gap closes via the new reproducible **`eval_spacenet --grayscale`** (PAN proxy). Done when the grayscale-proxy IoU is within a few % of RGB on the held-out set. *(Tooling shipped: `build_val_transform(grayscale=)` + `eval_spacenet --grayscale`.)* Lane: Akshat. — **IN PROGRESS (2026-07-02):** added the missing levers — (a) **radiometric aug** (`RandomGamma`) now rides with `grayscale_p>0` in `build_train_transform` (decolorizing alone left the −11 % gap; PAN differs in tone, not just colour); (b) a **per-epoch grayscale forget-check** in `finetune` (`_iou_on_pairs(grayscale=True)` → logged + saved to meta as `indian_gray_val_iou`) so a run can *watch* the gap close, not just measure it at the end. +3 CPU tests (169 total). **DONE (2026-07-02):** restored CUDA (torch 2.12.1+**cu126**, 3070 Ti) and ran the v3.2 fine-tune (v1 init + SpaceNet train + DeepGlobe anchor, frozen encoder, 512px/10ep, grayscale_p 0.7 + gamma). **v3.2 (`road_pan.pt`, epoch 8) is the best model on every axis** at deploy thresholds: RGB **0.4594** / grayscale **0.4177** / APLS **0.4987** — vs v3 **+2 % / +3 % / +14 %** (routing gain is the big one: colour-independent training → more connected roads). Grayscale (PAN proxy) gap **−9 %** (v1 −14 %, v3 −10 %) — narrowed but not yet "within a few %", so PAN robustness is improved, not solved. Released **[`a4-roadseg-v3.2`](https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3.2)** (deploy thr 0.52) as the new default; committed benchmark JSONs re-baselined to include v3.2 (APLS n=80 @0.50). |
| **A25** | ✅ | **Deployment + eval correctness fixes** *(Codex audit 2026-07-01, verified)* | — | — | Cheap, no-GPU correctness plumbing to do **before** any more training (a good checkpoint can look worse from wrong inference/eval): (1) `predict.py` `--threshold`/`--tile-size` default to **checkpoint `meta`** (currently hardcoded 0.5/256; `run_pipeline` already does this right); (2) `train.evaluate` accumulate **global TP/FP/FN** (not batch-mean) for paper-grade eval; (3) `run_pipeline.py` expose A10 **postprocess** flags; (4) `finetune.py` CLI expose `--grayscale-p`/`--cldice-weight`/`--num-workers` + a committed A23 **spacenet-train** mode (loads frozen held-out chips, trains on the rest) so A23 is reproducible without the `.tmp` launcher. Done when each is fixed + tested. — **DONE (2026-07-02):** predict.py threshold/tile-size default to checkpoint `meta`; `train.evaluate(reduction='global')` (TP/FP/FN over the whole set, now the default; `batch_mean` still available); `run_pipeline` `--postprocess/--min-component-size/--pp-close-radius`; `finetune.py` `--grayscale-p/--cldice-weight/--num-workers` + `--spacenet-corpus` mode (reproduces A23 from one CLI, reserves the frozen held-out chips). Full suite green. Lane: Akshat. |
| **A26** | ✅ | **Georeferenced + PAN inference reader (Cartosat prep)** *(Codex audit #6; deployment-critical)* | A24 | — | `predict.py`/`run_pipeline.segment` read via OpenCV → **drop GeoTIFF CRS/transform** → graph is pixel-space (routing distances are a guess). Cartosat-3 final data is georeferenced. **Fix:** rasterio reader path; if source has CRS/transform, write `data/interim/{aoi}/manifest.json` (P2 already consumes it); handle **1-band PAN → percentile-stretch → 3ch** + 3-band RGB/pan-sharpened; optional 0.25→0.5 m downsample to match training scale. Done when a georeferenced GeoTIFF (RGB **and** PAN) runs end-to-end with a metric manifest. — **DONE (2026-07-02):** `raster_io.read_image_any` (rasterio for `.tif`: **1-band PAN → 2–98%% stretch → 3ch**, ≥3-band RGB/pan-sharpened/MX → A16 `_to_rgb8`; cv2 fallback for jpg/png) + `write_manifest` → P2's `data/interim/{aoi}/manifest.json` (`transform`+`crs`, rebuilt via `Affine(*meta['transform'])`). Wired into `predict.py` + `run_pipeline.segment`. 4 tests incl. Affine round-trip. Cartosat PAN/RGB now runs end-to-end georeferenced. Lane: Akshat. |
| **A27** | ✅ | **Overlapped Hann-blended inference** *(Codex audit #2)* | — | — | `predict_large` tiles **without overlap** and stitches **binary** → roads can break at tile seams (worst exactly where the graph stage is sensitive → hurts APLS). **Fix:** `predict_prob`/`predict_large_prob` with stride ~256/384 windows, blend **probabilities** with a Hann/smooth window, threshold **once** on the assembled map. Done when held-out **APLS/connectivity** improves (or holds) vs the current tiling, IoU noted. — **DONE (2026-07-02):** `predict_prob` + `predict_large_prob` (75%%-overlap windows, Hann-weighted prob blend, threshold once) + `predict.py --blend/--stride`. 2 tests (coverage, [0,1], seam-consistent on uniform input). NB: the held-out SpaceNet tiles are exactly 512px = a single window, so blending only affects **large-image deployment** (Cartosat/stitched AOIs) — its APLS benefit will show there, not on the per-tile held-out set. Lane: Akshat. |
...
| **F5** | ✅ | **Draw-a-flood-polygon → multi-node failure simulation** | S11 | — | Folium Draw plugin; disable all nodes inside the drawn polygon; recompute RI + reroute. Done when drawing a polygon disables the enclosed nodes and updates RI/charts live. — **DONE** (Saanvi #84): `Draw` plugin → nodes `within` the polygon → multi-node `simulate_ablation`. (Akshat: aligned `SimulationResult` to the multi-node design on merge.) |
...
**2026-07-02 (Akshat — A24 ✅ v3.2 PAN-hardened released)**
- **Restored CUDA** on the 3070 Ti (env torch had reverted to cpu-only): `torch==2.12.1+cu126` (cu124 index tops out at 2.6; cu128 lacks 2.12 — cu126 was the matching build; driver 596.49/CUDA 13.2 covers it).
- **Ran the v3.2 fine-tune** (mirror A23 + grayscale_p 0.7 + `RandomGamma`, → `models/road_pan.pt`). Selected **epoch 8** (best RGB val, keeps DeepGlobe 0.633). Per-epoch grayscale log worked as designed — watched the gap track −14 %→−10 % across epochs.
- **Held-out verdict (512px, each model @ its deploy threshold):** v3.2 RGB **0.4594** / grayscale **0.4177** / APLS **0.4987** — beats v3 by **+2 % / +3 % / +14 %**, beats v1 by far more. The aug helped **routing most** (APLS +14 %): training off colour → more connected roads (pixels ≠ topology). Grayscale (PAN proxy) gap **−9 %** (v1 −14 %, v3 −10 %) — improved, not solved; PAN robustness is *better*, the modality gap isn't closed. DeepGlobe forget-check held.
- **Shipped:** [`a4-roadseg-v3.2`](https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3.2) (`road_pan.pt`, deploy thr **0.52**) as the new default; #97 (tooling) merged; committed benchmark JSONs re-baselined to include v3.2 (RGB/gray sweeps @512px, APLS n=80 @0.50 — supersedes the earlier n=50 @0.44 APLS). README/Evaluation/SETUP/Tracker updated.

**2026-07-02 (Akshat — A24 tooling: radiometric aug + per-epoch grayscale forget-check)**
- Started **A24** (Cartosat-PAN robustness). The two missing levers, both CPU-tested: (a) `build_train_transform` now adds **`RandomGamma`** whenever `grayscale_p>0` — v3 used `ToGray` alone and still had a −11 % grayscale gap, because PAN differs from RGB in *tone/contrast*, not just colour; (b) `finetune._iou_on_pairs` gained a `grayscale=` mode and the epoch loop now logs an **Indian grayscale IoU** (PAN proxy) + saves it to checkpoint meta (`indian_gray_val_iou`) — so a run can *watch* the gap close per epoch, not just at the end. +3 tests → **169 green**.
- **Blocked on GPU:** the local env's torch is now `2.12.1+**cpu**` (`torch.version.cuda=None`; no conda/venv, no other CUDA python) — the CUDA build A23 trained on is gone. The v3.2 PAN-hardened fine-tune (mirror A23's recipe: v1 init + SpaceNet train + DeepGlobe anchor, frozen encoder, 512px/10ep, but grayscale_p≈0.7 + gamma) is written and ready; needs a CUDA env restored (local cu124 reinstall for the 3070 Ti, or run on Kaggle/Colab per SETUP Path B). Awaiting Akshat's call on which.

**2026-07-02 (Akshat — v3.1 release: deploy threshold 0.44→0.50; SETUP.md synced)**
- **Acted on A21's recommendation.** Cut **[`a4-roadseg-v3.1`](https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3.1)** — *identical v3 weights*, only `meta["threshold"]` bumped 0.44→0.50 (the held-out sweep optimum). Free **+0.018 IoU** on real Indian GT (0.4311→0.4493); the pipeline reads its deploy threshold from checkpoint meta, so `predict`/`run_pipeline` pick it up with no flag change. Local `models/road_spacenet.pt` meta updated to match. README/Evaluation/SETUP now point at v3.1 as the deployed default (v3 kept as the original @0.44).
- **SETUP.md synced** (the /update-docs flag): fixed the Kaggle accelerator note (avoid P100 — our torch build fails on it; launch T4 from the UI) and the P1-inference block (v3.1 checkpoint name, `--blend`/`--postprocess`, GeoTIFF/PAN georef auto-manifest).
...
- **Logged the valid undone fixes:** **A25** (predict.py meta defaults, global-aggregated eval, run_pipeline postprocess, finetune CLI/repro — all cheap, no-GPU), **A26** (georef + PAN inference reader — Cartosat-critical), **A27** (overlapped Hann-blended inference — likely APLS gain). Audit also reinforces our existing verdicts: build from A23; avoid A8/A9/A11/A12 paths + TTA-default; hold the $350 GCP until cheap fixes land.

**2026-06-28 (Akshat — A17 ✅ complete (APLS added); v3 released)**
- **A17 → ✅** — added the topology half: `apls_eval.py` (mask→skeleton→graph via P2, scored with S7 `apls.py`). **Held-out APLS (n=50): v3 0.4147 vs v1 0.3844 → +0.030 (+7.9%).** So v3 beats v1 on **both** IoU (+14%) **and** routing (APLS +7.9%) — the truthful Indian benchmark (IoU + APLS, RGB + Cartosat-grayscale proxy) is now complete and v3 is validated on it. 3 tests; report `data/sample/spacenet_mumbai_apls.json`.
- **Released `a4-roadseg-v3`** (road_spacenet.pt) — https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3 — with the held-out RGB+grayscale IoU and the recipe. It is now the best Indian model; v1/v2 remain for reference.

**2026-06-28 (Akshat — A23 ✅ WIN: first model to beat v1 on real Indian GT)**
- **A23 DONE → ✅.** Supervised fine-tune of v1 on real SpaceNet-Mumbai labels (frozen encoder, DG anchor, grayscale-0.5 aug) peaked at epoch 6. **Honest held-out eval (real GT, 449 tiles, @0.44, 512px):**
  - **RGB: A23 0.4298 vs v1 0.3768 → +0.053 (+14%)** — also beats v2 (0.3727). DeepGlobe held (0.6802 ≥ v1 0.6314).
  - **Grayscale (Cartosat-PAN proxy): A23 0.3811 vs v1 0.3235 → +0.058.** Sensor gap shrank (A23 −11.3% vs v1 −14.1%); **A23-grayscale 0.3811 ≈ v1-RGB 0.3768** — Cartosat-robust.
- **The breakthrough after A8/A9/A11/A12 all failed:** the lever was *real in-domain labels + a truthful metric* (not architecture, not more techniques, not self-training on noisy OSM) — exactly the thesis. `road_spacenet.pt` is now the best Indian model. **Released as `a4-roadseg-v3`** (https://github.com/Akshat-Tiwari69/Trace/releases/tag/a4-roadseg-v3) (tag/name TBD). Next: APLS (A17), maybe a small encoder-unfreeze to push further.

**2026-06-28 (Akshat — A23 supervised SpaceNet fine-tune LAUNCHED)**
- **A23 → 🔄, training live on the 3070 Ti.** The honest replacement for the rejected A12: fine-tune **v1** on **real** SpaceNet-Mumbai labels (1758 train tiles, held-out 127 chips reserved) + DeepGlobe-2000 anti-forgetting anchor, **frozen encoder** (A6's proven recipe), **grayscale_p=0.5** aug (A24 Cartosat-PAN robustness), clDice off (8 GB safety), 512px batch 2, 10 ep → `models/road_spacenet.pt`. Reused `finetune.py` with 3 small committed adds: `finetune_pairs` override, `grayscale_p`, configurable `cldice_weight`; + `eval_spacenet.train_pairs` (held-out complement, leakage-tested). CPU-smoke-verified the full path; launched detached.
- **When done:** eval vs v1 on the held-out split in **both RGB and `--grayscale`** — release only if it beats v1's real-GT bar (0.3768). First experiment with a real shot at honestly beating v1 *and* addressing the Cartosat sensor gap in one run.

**2026-06-28 (Akshat — merged Saanvi's F3+F5 #84)**
- **F3 ✅ + F5 ✅** (Saanvi #84, integrated by Akshat). Her branch was 1 commit on `app.py` forked from old dev (before F4–F8); GitHub showed "66 files" but the real change was app.py only. Rebased onto dev (auto-merged textually) and **fixed two semantic breaks the auto-merge left:** (1) `SimulationResult.disabled_node` (singular) vs F5's `disabled_nodes` (tuple) constructor/readers → aligned the dataclass to multi-node; (2) `graph_from_features` used `@st.cache_resource` with an unhashable non-`_`-prefixed GeoDataFrame arg → `_features` skip-hash. Smoke-verified all of F3–F8 coexist (graph build, single+multi-node flood sim, build_map, both exports). **A `streamlit run` is the recommended final UI check** (bare-mode can't exercise the Draw-plugin / rerun flow). Saanvi's dashboard lane (F1–F8) is now complete.

**2026-06-28 (Akshat — Cartosat-3 sensor gap measured → A24 added)**
- **Confirmed the deployment sensor:** Cartosat-3 (Bhoonidhi sample products) = **PAN (panchromatic ~0.25 m grayscale)** + MX (1.13 m, 4-band) + MX-Merge (pan-sharpened). Provided at hackathon end (restricted) → we must generalise to it sight-unseen. Our entire train/eval stack is **RGB** (DeepGlobe, SpaceNet WorldView-3) — so there's an unaddressed **sensor-modality gap** on top of geography.
- **Measured the risk (PAN proxy):** v1 on held-out SpaceNet Mumbai, RGB vs grayscale input — **IoU 0.375 → 0.318 (−15 % rel)**. The model relies on colour cues Cartosat PAN won't provide; the real Cartosat gap is likely larger. So **0.375 is an optimistic ceiling for the actual deployment.**
- **Action:** added **A24** (sensor-robustness: grayscale + radiometric aug so the model is colour-independent) and made it a **requirement of A23**. Shipped reproducible tooling: `build_val_transform(grayscale=)` + **`eval_spacenet --grayscale`** so we can track the PAN gap closing as we add the aug. 1 new test; suite green.

**2026-06-28 (Akshat — A17 IoU benchmark live: FIRST real-GT Indian re-baseline)**
- **Built the held-out SpaceNet-Mumbai IoU benchmark** (`p1_segment/eval_spacenet.py`): frozen **chip-level** split (`data/sample/spacenet_mumbai_heldout_chips.json`, seed 17 → 127/637 chips, 449 tiles, never trained on) + IoU/Dice eval reusing `train.evaluate`. 4 unit tests; full suite green.
- **FIRST-EVER measurement on real Indian GT** (held-out, IoU @0.44, 512px): **v1 0.3768 ≈ v2 0.3727**. **v2 does NOT beat v1 on real GT** — the long-held "v2 is our best on Indian" belief was a **metric artifact** (v2's 0.336 was OSM-agreement on its *own* training tiles). On truth, A6's OSM fine-tune bought ~nothing. **v1 (0.377) is the honest bar A23 must beat.** Report committed: `data/sample/spacenet_mumbai_eval.json`.
- **Remaining for A17:** APLS (topology metric) — needs metre-correct mask→graph plumbing + S7 `apls.py`.

**2026-06-28 (Akshat — strategy pivot: SpaceNet-5 Mumbai = real Indian GT, no hand-labeling)**
- **A17 reframed; A23 added.** Hand-labeling vector centerlines (the old A17) is infeasible for the team (students) — and unnecessary: **SpaceNet-5 Mumbai (A16) already gives us professional human-drawn Indian GT** (2,207 tiles + raw vector centerline GeoJSONs at 0.3 m, `data/raw/spacenet/SN5_roads_train_AOI_8_Mumbai/`). **New A17:** hold out a fixed Mumbai TEST split (never trained on) as the **truthful Indian benchmark — IoU + APLS** (reuse S7 `apls.py`); re-baseline v1/v2 on it. **New A23:** replace the rejected OSM self-training (A12) with a **clean supervised fine-tune on the SpaceNet-Mumbai TRAIN split** (strong labels) measured on the held-out split — release only if it beats v1 on the real metric.
- **Why this matters:** A12's lesson was that **OSM-agreement is a misleading metric**. Crucial follow-on realization — **we have never measured anything on real Indian GT**: even A12's "loses to v1" verdict was scored on OSM-labeled `data/finetune` tiles. The SpaceNet-Mumbai held-out split fixes that for the first time, and makes A18 (SAM-Road++) credibly evaluable too. A17 flipped 🔒→⏳ (data already exists; fully automatable). The old hand-label tile-selector is built + parked on branch `akshat/A17-tile-selector`.

**2026-06-28 (Akshat — A10 mask post-processing done)**
- **A10 ✅** — `src/pipeline/p1_segment/postprocess.py`: `remove_small_components` (connected-component area filter — lossless on the large connected road network), `morphological_cleanup` (binary open/close), `otsu_threshold` (data-adaptive binarization utility), `prune_skeleton_spurs` (walk-from-endpoint, removes short dead-end branches), and a conservative `postprocess_mask` chain (defaults to small-component removal only, so thin 1-px roads aren't eroded). Wired into `predict.py` (`--postprocess`, `--min-component-size`, `--pp-close-radius`). **Done-criterion verified on a real Panaji tile (512², 8.4% road): connected components 136 → 2, IoU vs GT 0.9938 → 0.9999 (cleanup loss +0.0001, well under 0.005).** TDD: 9 unit tests; full suite 151 passing. (Observed a pre-existing flaky test `test_train_selftrain::test_unlabeled_dataset_yields_two_perturbed_views` — order/RNG-dependent, unrelated to A10, in the now-rejected A12 harness.)

**2026-06-28 (Akshat — F7+F8 integrated & merged; Tracker S-row dedupe)**
- **F7 ✅ + F8 ✅** (Saanvi's work, integrated onto `dev` by Akshat). The F7/F8 branches (#75/#76) were stacked on F6 and would have needed fragile 4-deep rebases over the squash-merged F6, so I integrated the net-new features directly: **F7** — `generate_summary_png()` (PIL one-page report: RI, CC-size, top-10 criticality table) + GeoJSON/PNG download buttons; **F8** — Okabe-Ito **colorblind-safe palette** (selected `#56B4E9`, disabled `#D55E00`, rerouted `#E69F00`), spinner messages on the cached compute, split `FileNotFoundError` vs parse-error handling. Fixed on integration: PIL default font is latin-1 → **ASCII-only report text** (same gotcha as the A12 log); kept the real P3 resilience CSV (F7's branch re-introduced the fabricated one). Smoke-tested (PNG valid, GeoJSON export, F4 SPOF + F6 side-by-side intact, palette renders). PRs #75/#76 to be closed as integrated.
- **Tracker hygiene fixed:** deduped §6 Shaivi — S3/S4/S5/S9/S10/S11 each had **two contradictory rows** (e.g. S5 was both "DONE" 🔄 and "🔒 blocked"). Now one authoritative row per task. **CORRECTION (later same day):** an earlier version of this note wrongly said S5/S9/S10/S11 were "done but not yet PR'd" — they are in fact **all merged** (S5 #54, S9 #55, S11 #56, S10 #58, plus batch #60, on 2026-06-26; code in `dev`: `simplify_polylines`, `BetweennessCache`, `percolation.py`, `flood.py`). Flipped those rows 🔄→✅. **So Shaivi's entire S1–S11 lane is COMPLETE**, and Saanvi's **F3** (dep S5) and **F5** (dep S11) are **unblocked and ready** — just not yet started. Nothing is blocked in either lane; the stale "PR pending" text had masked that S-work was already merged.

**2026-06-28 (Akshat — A12 self-training REJECTED; dashboard F4+F6 merged)**
- **A12 → ✅ REJECTED (honest negative).** Ran the self-training on the 3070 Ti in two configs. Val (margao+delhi) peaked at epoch 1 then declined both times: original λ→1.0 run **0.3545→…→0.2142**; gentler re-tune (λ→0.3, conf 0.85, min_pseudo 100) **0.3685→0.3104→0.2715**. **Held-out TEST eval (panaji+mumbai_bandra+bengaluru, 110 tiles, IoU@0.44, 384px, single-view): v1 0.2810 · A12 re-tune-best 0.2578 · A12 orig-best 0.2426 → A12 LOSES to v1 in both.** Mean-teacher self-training on the OSM-weak Indian corpus adds no real signal on the deployment target (joins A8/A9/A11). **Big lesson: the OSM-agreement val metric is misleading** — a higher val IoU still lost on the held-out TEST cities, so we can't select on it. Not released; v1 stays baseline, v2 stays deployed. **Pivot → A17** (hand-labeled Indian GT → truthful APLS/IoU) **/ A18** (graph-first SAM-Road++). Also logged review follow-ups **A19–A22** (resume, de-CPU `refine_pseudo`, eval threshold sweep, pseudo-label knobs).
- **Merged dashboard PRs:** #73 **F4 ✅** (single-points-of-failure: articulation nodes + critical bridges) and #74 **F6 ✅** (before/after side-by-side + resilience-curve chart) — both reviewed + fixed-on-merge (F4: robust `_truthy()` flag parsing; F6: **swapped a hand-fabricated sample resilience CSV for the real P3 output** so the curve matches P3 numbers, rebased onto F4). Plus #71 (A12 review triage) and #72 (entry-point "record what you find" principle + CLAUDE≡AGENTS sync).
- **⚠️ Tracker hygiene flagged (not yet fixed):** §6 Shaivi has **duplicate/contradictory S-rows** — S3/S4/S5/S9/S10/S11 each appear twice (e.g. S5 listed both "DONE" 🔄 and "🔒 blocked"). The duplicates are stale; the real state is S3/S4 merged, S5/S9/S11 **done but PR-pending**. Needs a dedupe pass (Shaivi's lane — flagged to the team).
