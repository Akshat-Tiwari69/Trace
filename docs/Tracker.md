...
- **The breakthrough after A8/A9/A11/A12 all failed:** the lever was *real in-domain labels + a truthful metric* (not architecture, not more techniques, not self-training on noisy OSM) — exactly the thesis. `road_spacenet.pt` is now the best Indian model. Release pending user OK (tag/name TBD). Next: APLS (A17), maybe a small encoder-unfreeze to push further.
