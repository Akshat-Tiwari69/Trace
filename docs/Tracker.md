...
| **S8** | 🔄 | **Articulation points & bridges as a criticality layer** | — | F4 | Add `nx.articulation_points` + `nx.bridges` to `criticality.py`; emit `is_articulation` / `is_bridge`. Done when these columns are emitted and validated on sample (known cut node detected), tests added. — **DONE** (`criticality.annotate_cut_structure`, wired into `analyze`): flags true single-points-of-failure (cut nodes / cut edges) as a structural criticality layer distinct from betweenness. Emits `is_articulation` (node → criticality.csv + GeoJSON) and `is_bridge` (edge → GeoJSON; **distinct from `is_bridged`** = healing-inferred). Validated on sample: **239 articulation points (38%), 306 bridge edges (41%)**. 5 unit tests (barbell cut node/edge, cycle has none, disconnected, rank_table column). **PR pending (off `dev`).** |
