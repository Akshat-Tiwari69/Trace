...
| **S7** | 🔄 | **APLS + topology validation against OSM** | — | E4 | Port CosmiQ `apls` + connectivity ratio comparing the healed graph to OSM ground truth. Done when APLS is reported for the sample graph in `Evaluation.md` and re-runnable via a CLI. — **DONE** (`p3_analysis/apls.py` + CLI): self-contained, **densified, symmetric node-based APLS** (no heavy CosmiQ dep); fetches+commits an OSM ground-truth graph (`{aoi}_osm_truth.geojson`) so it's offline-reproducible. Sample **APLS = 0.45** (gt→prop 0.34, prop→gt 0.67) on the deliberately occluded S1 sample vs SpaceNet baseline ≈0.49; reported in `Evaluation.md`. 4 unit tests (identical=1, broken-path<1, no-correspondence=0, detour partial). **PR pending (off `dev`).** |
