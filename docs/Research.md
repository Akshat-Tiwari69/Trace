...
**A18 implementation note (2026-07-12 IST).** The upstream SAM-Road++ snapshot (`44c6e7b`) has no licence, configs, or reproducible dependency lock, so the spike is **local research-only**; do not redistribute its code or derived weights without upstream licence clarification. Its SpaceNet loader also assumes the original SpaceNet-3 graph pickles use Cartesian `y`-up coordinates and applies `y = 400 - row`. Our SpaceNet-5 converter writes ordinary raster `(row, col)` coordinates, so reusing that transform vertically mirrored graph supervision: only 15–24% of sampled graph nodes landed within 2 px of their road mask (median error 10–15 px). The local adapter must instead map **`x = col, y = row`**. After correction, all sampled validation nodes landed on-mask (100%, median/max distance 0 px). Keep this invariant as a pre-training assertion; a silent coordinate convention mismatch can make a graph architecture appear to fail while the labels are wrong. A corrected 256 px / batch-1 / frozen-SAM GPU fast-dev smoke is pending after A41b; the pre-fix smoke established only hardware feasibility (0.43 GiB peak reserved VRAM), not topology quality.
