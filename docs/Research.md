...
**A18 implementation contract (2026-07-12).** The inspected SAM-Road++ snapshot (`44c6e7b`) has no licence, configs, or reproducible dependency lock, so the spike remains local research-only; do not redistribute upstream code or derived weights without licence clarification. Its SpaceNet loader assumes Cartesian `y`-up graph coordinates and applies `y = 400 - row`. Our SpaceNet-5 converter writes raster `(row, col)` coordinates, so that transform vertically mirrored supervision: only 15–24% of sampled nodes landed within 2 px of their road mask (median error 10–15 px). The local adapter must use **`x = col, y = row`**. After correction, sampled validation nodes were 100% on-mask (median/max error 0 px). Keep this alignment invariant as a pre-training assertion; otherwise a coordinate mismatch can masquerade as architecture failure.
