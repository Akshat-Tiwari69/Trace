...
> ## Fix status — A36 (branch `akshat/a36-s-fixes`, 2026-07-04)
> **Every S-effort finding in this report is fixed** on the A36 branch (including the S-parts of S–M/split findings), verified by the full suite (**194 passed**, +21 new regression tests in `tests/test_s_fixes.py` + `tests/test_sample_artifacts.py`). Deliberate exceptions, per the report's own guidance:
> - **§3 experiments/ quarantine** — the finding's fix text says "no action required now"; deferred to a dedicated hygiene pass to keep this PR reviewable.
> - **§4 flood/percolation spatial index** — deferred by the report until city-scale runs exist.
> - **§5E geopandas major alignment & Python pin** — documented (deploy/README "Python & dependency matrix") instead of changed; pin flips are their own risk and need a joint dev+prod test.
> - **§6 Caddy rate limiting** — requires the third-party caddy-ratelimit module; body-size cap added, plugin noted as follow-up.
>
> **Every M-effort finding is now also fixed** (A36 M-pass, full suite **213 passed**, +19 new tests in `tests/test_eval_harness.py` + `tests/test_m_fixes.py`):
> - **ML eval rigor (§3):** paired-bootstrap CIs on the APLS delta (`stats.py`, `apls_eval --compare`), honest threshold select/report split (`eval_spacenet --honest-threshold`), golden harness regression tests.
> - **Graph (§4):** road `width_m` via distance transform (topology-preserving), false-bridge edge-crossing rejection (STRtree), auto k-sampling in the interactive path + bounded (LRU) betweenness cache.
> - **Orchestration/provenance (§5A/§5B):** model-provenance record threaded P1→P2→P3 (`provenance.py`, graph metadata, `{aoi}_provenance.json`); structured logging + per-stage timings + `{aoi}_run.json`; idempotent reruns (`--force`/`--from-stage`/skip-if-fresh); one `PipelineConfig` (JSON/YAML) as the source of truth.
> - **Dashboard (§2/§5C/§5F):** map-viewport stability (stable key + center/zoom round-trip — the §2C P0), compound-closure accumulation with removable chips, rankings table row-selection → recentre with a ProgressColumn score bar, a Methodology tab surfacing the eval JSONs, a robust Modal client (concurrency semaphore + cold-start retry/backoff), and minimal observability (structured logs + usage counter).
> - **Deliberately deferred M sub-items, with reasons:** the §2C `folium.GeoJson` vectorised edge layer + map-render caching (a perf-at-10k-edges item; the demo is ~500 edges, and the rewrite is high-risk for no current-scale benefit); §2C **DualMap** side-by-side sync (the report itself flags DualMap as flaky with streamlit-folium and offers "drop the mode" as the alternative — left as-is, unsynced, rather than ship a flaky sync); the §2C legend-bar **merge** (pure cosmetic — the S-pass already made the legend truthful); §3 **topology-loss retest** ("M only if retested" — needs a GPU retrain experiment, not a code change); §6 `weights_only` **meta migration** ("not urgent" — re-serialising the deployed v3.2 checkpoint format risks breaking its load path). Remaining **L items** (MultiGraph end-to-end, probability-map-aware healing, upload→graph loop, pre-tiling, filesystem job queue) are unchanged — see §8/§9.
>
>
> **L-effort pass (A36, full suite 218 passed, +5 tests in `tests/test_l_fixes.py`):** the two fully-achievable, highest-value L items are done to production quality —
> - **Close the upload loop (§2B):** `src/app/upload_analysis.py` runs the CPU P2→P3 pipeline in-process on an uploaded mask and the dashboard now shows a *real* resilience analysis of your own imagery (junctions/links counts, worst-single-failure efficiency, an image-space network overlay with the #1 chokepoint ringed, and a criticality table) — converting the app from "viewer of one canned city" to the actual product. Honest about coordinates: uploads aren't georeferenced, so it renders over the image (with a GSD slider) rather than faking a world-map placement; fail-loud on an empty mask.
> - **Pre-tiling / windowed inference (§5H):** `raster_io.iter_windows`/`raster_dimensions`/`raster_georef` + `model.predict_large_raster` stream a large raster off disk one window at a time (never materialising the full RGB image — the OOM point), OR-merging into a single binary mask; auto-enabled in `run_inference` above `WINDOWED_THRESHOLD_PX` (4096) with `--windowed`/`--window-px` CLI control. Unblocks pilot-scale AOIs.
> - **Deferred L items, with reasons:** **SAM-Road++ topology-first spike (§3)** — genuinely blocked: needs a GPU, SAM weights, and training/eval runs that cannot be executed or validated in this environment (it is explicitly a "spike to justify or kill a pivot," not a production feature; the A16 corpus + A17 APLS harness it would reuse already exist, so it is one GPU session away). **MultiGraph migration end-to-end (§4 P0)** — deferred deliberately: the silent parallel-edge data loss is *already mitigated* (shorter-branch-keep + counted warning from the S/M passes), so the residual value is representing genuinely-parallel roads, which requires a milestone-sized refactor across skeleton/simplify/healing/criticality/resilience/serialization/dashboard with high regression risk — not worth destabilising a green pipeline in the same pass as the upload loop. **Probability-map-aware healing (§4 L)** — the corridor-crossing half of the finding is done (M pass, STRtree edge-crossing rejection); the remaining probability-map plumbing is a P1↔P2 contract change best sequenced with the MultiGraph work. **Filesystem job queue (§5H, M–L)** — infrastructure whose need is created by the new upload loop; scoped as the next deploy-hardening step. See §8/§9 for the sequencing.
>
> **Operator actions still needed after merge** (can't be done from the repo): redeploy Modal (`modal deploy deploy/modal_app.py` — image now checksum-verified, hardened, cost-capped); delete the port-8501 ingress rule in the Oracle console + drop the iptables ACCEPT; install the updated systemd unit (loopback bind) + journald drop-in + Caddyfile body cap; `chmod 600` the env file holding `MODAL_SEG_KEY`; sanity-check blended-inference CPU latency on the ARM box (blend is now the default path).
...
#### [P1] No value proposition or guided entry — first 5 seconds are unexplained
...
#### [P1] Brand header lives inside the 35% right column, not the page top
...
#### [P2] "LIVE" pulsing chip is a false affordance
...
#### [P1] No About/Methodology view — an entire persona has no entry point
...
#### [P2] Two-column scroll imbalance on 1366×768; panels never stack as Design.md promises
...
#### [P2] Disabled "Region" selectbox signals unfinished software
...
#### [P1] Scenario dropdown is decorative — "Accident" and "Road closure" behave identically
...
#### [P0] GPU inference re-fires on every Streamlit rerun — every click re-pays 5–240 s
...
#### [P1] The upload — the product's headline pitch — is buried, conditional, and dead-ends
...
#### [P1] No staged progress or cold-start messaging during the long call; the one warning that exists disappears at the worst moment
...
#### [P2] No client-side validation before shipping the image to the GPU; constraints not stated up front
...
#### [P2] Upload error states: raw exception surfaced, no retry, and an uncaught-traceback path
...
#### [P0] Every selection, toggle, or scenario change destroys the map viewport (zoom/pan reset)
...
#### [P1] FastMarkerCluster is the wrong tool for ~40 markers — it hides the critical junctions it's supposed to showcase
...
#### [P2] Legend inaccuracies and split legends
...
#### [P2] Clicking empty map / non-critical junction is silently ignored
...
#### [P2] Map rebuild is uncached and per-edge; will not survive 10k+ edges
...
#### [P2] Side-by-side comparison mode: unsynchronized panes, doubled clutter
...
#### [P0] Resilience Index delta color is semantically inverted — a resilience *drop* renders green
...
#### [P2] Resilience Index is a naked number with no context, while benchmark data sits unused on disk
...
#### [P2] "Travel impact" line chart plots exactly two points — a line through (0,0) and (1,Δ)
...
#### [P2] Top critical junctions table: not linked to the map, only 5 rows, scores stringified
...
#### [P2] Compound disasters (UserJourney Flow B) are impossible outside Flood-draw
...
#### [P2] Flood-mode residue: leaving the Flood scenario keeps the flooded ablation with contradictory messaging
...
#### [P2] Map-click selection: double rerun flicker and a stale-click dead zone
...
#### [P3] `st.success` for a destructive event; disconnection message under-dramatized
...
#### [P3] Exports computed eagerly on every rerun; PNG palette diverges from the design system
...
#### [P2] Design.md drift: font, palette, and the mandated colorblind-safe ramp all diverge undocumented
...
#### [P2] Color tokens defined once in CSS but duplicated as hardcoded hex across 6+ Python sites
...
#### [P2] Accessibility gaps: mouse-only map, tooltip-only data, tiny uppercase section labels
...
#### [P3] Dead code / hygiene in the shipped file
...
#### [P3] Page chrome: no footer, no menu metadata, emoji favicon
- **Where:** `app.py:963`
...
#### [P3] `st.cache_resource` graph keyed on nothing — a footgun for the upload-to-graph future
...
#### [P0] Defaults/docstrings hardcode the superseded v1 checkpoint — three releases stale
...
#### [P1] Uniform random crop wastes signal on a severely imbalanced task
...
#### [P1] No confidence intervals / significance anywhere in the promotion pipeline
...
#### [P1] Deploy threshold tuned on the same held-out set used for the reported number
...
#### [P1] `_read_val_pair` lru_cache is unbounded and never invalidates on data change
...
#### [P2] The default inference path still has no overlap/blending — the seam-artifact fix exists but is opt-in
...
#### [P2] No golden regression test on the promotion harness itself
...
#### [P2] Grayscale proxy ≠ real Cartosat PAN — the "-9% gap" is unvalidated against the real sensor
...
#### [P3] Implementation hygiene (all S)
...
#### [P0] `sknw.build_sknw(..., multi=False)` silently drops parallel edges between the same junction pair
...
#### [P0] Zero-length edges guarded only at one call site, not as a graph-wide invariant
...
#### [P1] Betweenness/global-efficiency exact by default in the interactive path; cache fully invalidates per ablation
...
#### [P1] Skeletonization discards road width — `medial_axis` would give it for free
...
#### [P1] K-sampled efficiency resamples independent source sets at every ablation step — avoidable variance in the resilience curve
...
#### [P1] Douglas-Peucker with `preserve_topology=False` on curved Bézier bridge geometry can self-intersect
...
#### [P2] `rank_table` sorts purely by betweenness — articulation-point SPOFs with modest traffic rank far down the CSV
...
#### [P2] `consolidate_nearby_nodes` merges transitively through chains of sub-tolerance edges — cluster diameter unbounded
...
#### [P2] `prune_short_stubs` can hit `max_iter=20` and silently under-prune
- **Where:** `simplify.py:76–93`. **Fix:** return/log whether the cap was hit. **Effort:** S

#### [P2] `build_osm_truth` (APLS ground truth) has no timeout/retry/graceful failure for new AOIs
...
#### [P2] Point-in-polygon and demand-weighting are O(V) unindexed Shapely loops
...
#### [P3] Silent pixel-space fallback when no georeference manifest exists (`resolution_m=1.0` default)
...
#### [P3] No round-trip test against the actual committed `data/sample/*_graph.geojson`
...
#### [P0] No AOI-id sanitization — path traversal / collision from user or CLI input
...
#### [P0] Non-atomic artifact writes — a crashed stage leaves a half-written mask/CSV/GraphML the next stage happily consumes
...
#### [P1] Model provenance is dropped — you cannot answer "which checkpoint produced this mask/graph?"
...
#### [P2] Manifest at `interim/{aoi}/manifest.json` but mask at `interim/{aoi}_mask.png` — split-brain layout
...
#### [P2] No idempotency / resume — a rerun redoes every stage; a mid-pipeline failure restarts from P1
...
#### [P1] Zero error handling / logging in the orchestrator — failures are raw tracebacks, progress is `print()`
...
#### [P2] Config sprawl: the same tunables duplicated across `run()`'s 17 params, `GraphConfig`, and three argparse blocks
...
#### [P3] No multi-tile input handling — a large GeoTIFF AOI is one monolithic in-RAM array
...
#### [P0] Disk/RAM exhaustion is unbounded — no upload cap, no log rotation, no disk alarm
...
#### [P1] Upload concurrency: N simultaneous uploads = N cold Modal containers + N blocked 240 s session threads
...
#### [P2] No healthcheck; the 2-minute auto-update timer deploys raw `dev` with no smoke test and no rollback
...
#### [P3] `git pull --ff-only` freezes the box permanently if the checkout ever diverges
...
#### [P1] No retry, no payload cap, and HTTP 200 returned for auth/validation failures
...
#### [P1] No cost controls on the Modal function — unbounded scale-up on student credits
...
#### [P1] Torch version skew across training (2.12.1+cu126), local predict, and Modal (2.4.1) — unpinned and undocumented
...
#### [P2] No checksum on the model download at Modal image build
...
#### [P0] No CI — ~30 test modules, zero automation; nothing gates the auto-deploying `dev` branch
...
#### [P1] No contract test on the committed `data/sample/` artifacts — the deployed app's only data source is unguarded
...
#### [P1] Essentially no observability — no structured logs, no error tracking, no usage signal
...
#### [P2] TRD/Schema promise things the code doesn't do
...
#### [P3] `predict.py:12–13` docstring says multiband GeoTIFF is "out of scope" — but `read_image_any` (A26) already handles PAN + multispectral
...
#### [P0] Streamlit port 8501 exposed directly to the internet, bypassing Caddy/TLS entirely
...
#### [P0] Modal endpoint has no per-request size cap or rate limit — cost abuse / DoS via oversized or repeated payloads
...
#### [P1] Shared-secret compared with `!=` (non-constant-time), never rotated/expires
...
#### [P1] No upload size/type guard before PIL decode — decompression-bomb and CPU/GPU amplification
...
#### [P2] `torch.load(..., weights_only=False)` for all checkpoint loading, including the Modal-baked model
...
#### [P2] Verbose exception surfaced to end users on inference failure
...
#### [P2] Pillow is untracked in both requirements files despite decoding untrusted images
...
#### [P0] `analyze()` never checks the P2 graph is non-empty — a failed segmentation yields "no critical nodes," not an error
...
#### [P0] `resilience_index` returns 0.0 silently when baseline efficiency is 0 — indistinguishable from "network destroyed"
...
#### [P0] Modal response body never validated before decode (error-propagation angle of §2B's finding)
...
#### [P1] `load_checkpoint` silently defaults to `mit_b0`/`unet` when checkpoint `meta` is missing or partial
...
#### [P1] Flood polygon that selects zero nodes gives no feedback — identical to "not drawn yet"
...
#### [P1] Pipeline exit code is always 0 — CI/scripts can't detect a broken run
...
#### [P2] APLS returns a *perfect 1.0* when every sampled pair is unreachable
...
#### [P2] `rank_table` reads pre-annotated fields with silent `False` defaults
...
#### [P2] Missing georeference manifest is indistinguishable from intentional pixel-space mode
...
#### [P2] Corpus-build loop swallows per-city failures with no aggregate threshold
...
#### [P3] Massachusetts converter silently skips unreadable files (dead-experiment tool; only if ever reused)
